...
tutorial_vision::CircleDetectResult circle_detect_result;
void circle_cb(const tutorial_vision::CircleDetectResult::ConstPtr &msg) {
    circle_detect_result = *msg;
}
...
int main(int argc, char **argv) {
    ros::init(argc, argv, "accurately_land_node");
    ros::NodeHandle nh;
    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("mavros/state", 1, state_cb);
    ros::Subscriber local_pose_sub = nh.subscribe<geometry_msgs::PoseStamped>("mavros/local_position/pose", 1, pose_cb);
    ros::Subscriber circle_sub = nh.subscribe<tutorial_vision::CircleDetectResult>("circle_detect_result", 1, circle_cb);
    ...
    geometry_msgs::Point parking_target;
    ros::NodeHandle param_nh("~");
    parking_target.x = param_nh.param("parking_x", 0.);
    parking_target.y = param_nh.param("parking_y", 0.);
    parking_target.z = param_nh.param("parking_z", 1.);
    ...
    while (ros::ok()) {
        ...
        switch (fsm_state) {
            ...
            case 2:  // Takeoff state
                if (current_pose.pose.position.z > parking_target.z) {
                ...
                break;
            case 3:  // Move state
                if (getLengthBetweenPoints(parking_target, current_pose.pose.position) > 0.3) {
                    last_srv_request = ros::Time::now();
                }
                if (ros::Time::now() - last_srv_request > ros::Duration(1.5)) {
                    fsm_state = 4;  // goto wait for circle state
                } else {  // PID control
                    twist.twist = get_pid_vel(parking_target);
                }
                break;
            case 4:  // Wait for circle state
                if (ros::Time::now() - circle_detect_result.header.stamp < ros::Duration(0.5) &&
            ...
            case 5:  // Land state
                if (current_state.mode == "AUTO.LAND") {
                    ...
                    geometry_msgs::Point err;
                    err.x = circle_detect_result.width / 2.0 - circle_detect_result.circles[0].center_x;
                    err.y = circle_detect_result.height / 2.0 - circle_detect_result.circles[0].center_y;
